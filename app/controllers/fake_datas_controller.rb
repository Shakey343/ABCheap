class FakeDatasController < ApplicationController
  def create
    ...
    url_bus = "https://uk.megabus.com/journey-planner/journeys?days=1&concessionCount=0&departureDate=#{earliest_start_date_bus}&destinationId=#{destination_ids[0]}&inboundOtherDisabilityCount=0&inboundPcaCount=0&inboundWheelchairSeated=0&nusCount=0&originId=#{origin_ids[0]}&otherDisabilityCount=0&pcaCount=0&totalPassengers=1&wheelchairSeated=0"
    url_train = "https://ojp.nationalrail.co.uk/service/timesandfares/#{origin_ids[1]}/#{destination_ids[1]}/#{earliest_start_date_train}/#{earliest_start_time}/dep"
